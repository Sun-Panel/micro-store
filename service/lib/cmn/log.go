...
// TODO(GgoCoder) 日志轮转
func InitLoggerOld(fileName string, level zapcore.LevelEnabler) *zap.SugaredLogger {
...
func InitLoggerNew(logDir string, level zapcore.LevelEnabler, maxBackups int, maxAge int) *zap.SugaredLogger {
	// 创建一个配置
	...
	// 获取当前小时
	// currentHour := time.Now().Format("2006-01-02")

	// 设置日志文件名格式
	filename := fmt.Sprintf("%sapp_sun_panel_auth.log", logDir)
