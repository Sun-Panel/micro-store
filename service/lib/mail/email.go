...
// 发送邮件
func SendMail(emailer *Emailer, contentInfo ContentInfo) error {
	...
	// 替换模板变量
	for key, value := range contentInfo.RespaceArg {
		contentInfo.Title = strings.ReplaceAll(contentInfo.Title, key, value)
		contentInfo.Body = strings.ReplaceAll(contentInfo.Body, key, value)
	}

	m.SetHeader("From", m.FormatAddress(emailer.EmailInfo.Username, contentInfo.SendName))
	//这种方式可以添加别名，即“XX官方”
	//说明：如果是用网易邮箱账号发送，以下方法别名可以是中文，如果是qq企业邮箱，以下方法用中文别名，会报错，需要用上面此方法转码
	//m.SetHeader("From", "FB Sample"+"<"+mailConn["user"]+">") //这种方式可以添加别名，即“FB Sample”， 也可以直接用<code>m.SetHeader("From",mailConn["user"])</code> 读者可以自行实验下效果
	//m.SetHeader("From", mailConn["user"])
	m.SetHeader("To", contentInfo.MailTo...)  //发送给多个用户
	m.SetHeader("Subject", contentInfo.Title) //设置邮件主题
	m.SetBody("text/html", contentInfo.Body)  //设置邮件正文
