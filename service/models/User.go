...
type User struct {
	...
	StoreEmail   string `gorm:"type:varchar(50)" json:"storeEmail"` // 商城平台的邮箱账号
...
// 更新用户基于id
// 支持：name,autograph,header_image,status,role,mail,token,password,username,gender,lang,system_lang,time_zone
func (m *User) UpdateUserInfoByUserId(user_id uint, updateInfo map[string]interface{}) error {
	...
	if v, ok := updateInfo["lang"]; ok {
		data["lang"] = v
	}
	if v, ok := updateInfo["system_lang"]; ok {
		data["system_lang"] = v
	}
	if v, ok := updateInfo["time_zone"]; ok {
		data["time_zone"] = v
	}
