...
// LatestLowVersion 由 GetAppListWithLang 的子查询填充（最新版本要求的最低主应用版本）
type MicroAppListItem struct {
	...
	LatestLowVersion  string `json:"latestLowVersion,omitempty"`    // 最新版本最低支持的主应用版本（子查询填充）
}
...
func (m *MicroApp) GetAppListWithLang(db *gorm.DB, opts MicroAppListWithLangQueryOpts) ([]MicroAppListItem, int64, error) {
	...
	// 子查询：获取最新审核通过版本的最低主应用版本要求（直接从 low_version 字段读取）
	latestLowVersionSubQuery := db.Model(&MicroAppVersion{}).
		Select("low_version").
		Where("app_record_id = micro_app.id").
		Where("status = ? AND offline_type = ? AND deleted_at IS NULL", 1, 0).
		Order("created_at DESC").
	...
	query := db.Table("micro_app").
		Select(`micro_app.*,
			...
			COALESCE((?), '') as latest_version,
			COALESCE((?), '') as latest_low_version`, latestVersionSubQuery, latestLowVersionSubQuery).
