...
func (m *MicroAppDownload) GetDownloadCountByAppId(db *gorm.DB, appId uint) (int64, error) {
	var count int64
	err := db.Model(&MicroAppDownload{}).Where("app_id = ?", appId).Count(&count).Error
	return count, err
}
...
func (m *MicroAppDownload) CheckUserDownloaded(db *gorm.DB, userId uint, appId uint) (bool, error) {
	var count int64
	err := db.Model(&MicroAppDownload{}).Where("user_id = ? AND app_id = ?", userId, appId).Count(&count).Error
	return count > 0, err
}
