...
import Edit from './TemplateEdit/index.vue'
import { templateDeletes as deletesRequest, getTemplateList as getListRequest } from '@/api/admin/message'
...
const info = ref<Message.TemplateInfo | null>(null)
const dialog = useDialog()

const createColumns = ({
  update,
}: {
  update: (row: Message.TemplateInfo) => void
}): DataTableColumns<Message.TemplateInfo> => {
  return [
    {
      title: '模板名称',
      key: 'name',
...
const list = ref<Message.TemplateInfo[]>()

const columns = createColumns({
  update(row: Message.TemplateInfo) {
    handleEdit(row)
  },
})
...
function handleEdit(row?: Message.TemplateInfo) {
  if (row)
    info.value = row
...
async function getList(page: number | null) {
  ...
  const { data } = await getListRequest<Common.ListResponse<Message.TemplateInfo[]>>()
...
<template>
  <div>
    ...
    />
    <Edit v-model:visible="editDialogShow" :info="info as Message.TemplateInfo " @done="handelEditDone" />
  </div>
</template>
